  ...
  for (int pid = 0; pid < paragraphNum; ++ pid) {
    ...
    for (int sid = 0; sid < stnsNum; ++sid) {
      ...
      if (str_type == LTP_SERVICE_NAME_POSTAG
          ...
          || str_type == LTP_SERVICE_NAME_SEMDEPPARSE
      ...
      // sdp
      if (str_type == LTP_SERVICE_NAME_SEMDEPPARSE
          || str_type == LTP_SERVICE_NAME_ALL) {
        // get by word
      }
      ...
      for (int wid = 0; wid < vecWord.size(); ++wid) {
        ...
        if (str_type == LTP_SERVICE_NAME_SEMDEPPARSE
          ...
          std::vector<std::string> vecRelate;
          xml.GetLSTMSemanticParsesFromSentence(pid, sid, wid, vecParent, vecRelate);
          if (vecParent.size() != 0) {
            for (int sem_id = 0; sem_id < vecParent.size(); ++sem_id) {
              Json::Value sem;
              sem["id"] = sem_id;
              sem["parent"] = vecParent[sem_id];
              sem["relate"] = vecRelate[sem_id];
              sems.append(sem);
            }
          } else {
            sems.resize(0);
...
static int Service(struct mg_connection *conn) {
  ...
  if (!strcmp(ri->uri, "/ltp")) {
    ...
    if (str_type == LTP_SERVICE_NAME_SEGMENT){
      ...
      int ret = 0;
#ifdef USESDPTREE
      ret = engine->semantic_parser(xml4nlp);
      if (0 != ret) {
        ErrorResponse(conn, static_cast<ErrorCodes>(ret));
        return 0;
      }
